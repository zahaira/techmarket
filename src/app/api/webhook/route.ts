...
// Si la clé Stripe n'est pas définie (build/demo), on retourne un mock
if (!process.env.STRIPE_SECRET_KEY) {
  console.warn("⚠️ Stripe key missing — using mock endpoint for build.");
}

export async function POST(req: NextRequest) {
  // ⚡ Mode build/demo : mock response
  ...
  const stripe = new Stripe(process.env.STRIPE_SECRET_KEY);
  const sig = req.headers.get("stripe-signature")!;
  const body = await req.text(); // important: raw body pour vérifier le webhook
  ...
  try {
    ...
    switch (event.type) {
      case "payment_intent.succeeded": {
        ...
        const products = JSON.parse(paymentIntent.metadata.products || "[]");
        console.log("✅ Paiement réussi pour:", products);
        // Mettre à jour la DB ici si nécessaire
        break;
      }
      case "payment_intent.payment_failed":
        console.log("❌ Paiement échoué :", event.data.object);
